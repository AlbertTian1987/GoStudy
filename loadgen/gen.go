...
func NewGenerator(
	...
	fmt.Printf("Passed. (timeoutNs=%v, lps=%d, durationNs=%v)\n",
...
func (gen *myGenerator) init() error {
	fmt.Println("Initializing the load generator ...")
	...
	gen.tickets = tickets
	fmt.Printf("Initialized (concurrency=%d)\n", gen.concurrency)
	return nil
}
...
func (gen *myGenerator) Start() {
	...
	go func() {
		time.AfterFunc(gen.durationNs, func() {
			fmt.Println("Stopping load generator ...")
			gen.stopSign <- 1
		})
	}()
	//初始化完结信号通道
	...
	go func() {
		...
		fmt.Printf("Stopped. (callCount=%d)\n", callCount)
	}()
}
	...
	go func() {
		...
		timer := time.AfterFunc(gen.timeoutNs, func() {
			...
			result := &lib.CallResult{
				...
				Msg:  fmt.Sprintf("Timeout! (expectd: < %v)", gen.timeoutNs),
			}
			gen.sendResult(result)
		})
...
func (gen *myGenerator) sendResult(result *lib.CallResult) bool {
	...
	fmt.Printf("Ignore result: Id=%d, Code=%d, Msg=%s, Elapse=%v\n",
...
func (gen *myGenerator) interact(rawReq *lib.RawReq) *lib.RawResp {
	...
	if err != nil {
		errMsg := fmt.Sprintf("Sync call Error: %s.\n", err)
		rawResp = lib.RawResp{
			Id:     rawReq.Id,
